 ...
 * @author ivyzhang , spring 2022 for CS10
 */
...
public class Editor extends JFrame {
    ...
    public void drawSketch(Graphics g) {
        // TODO: YOUR CODE HERE
        for (Shape shape : sketch.getMap().values()) {
            shape.draw(g);
        }
        if (curr != null) {
            curr.draw(g);
    ...
    private void handlePress(Point p) {
        ...
        if (mode == Mode.DRAW) {
            ...
            for (Shape shape : sketch.getMap().values()) {
                if (shape.contains(p.x, p.y)) {
                    this.curr = shape;
                    comm.sendRequest("recolor" + " " + sketch.getShapeID(shape) + " " + this.color.getRGB());
                    repaint();
                }
            }
            ...
            for (Shape shape : sketch.getMap().values()) {
                if (shape.contains(p.x, p.y)) {
                    this.curr = shape;
                    comm.sendRequest("delete" + " " + sketch.getShapeID(shape));
                    repaint();
                }
            }
        }
    ...
    private void handleDrag(Point p) {
        if (mode == Mode.DRAW && curr!=null) {
            ...
            if (curr != null && moveFrom != null && curr.contains(p.x, p.y)) {
                comm.sendRequest("move" + " " + sketch.getShapeID(curr) + " " + (p.x - moveFrom.x) + " " + (p.y - moveFrom.y));
                moveFrom = p;
            }
            repaint();
        }
    ...
    private void handleRelease() {
        // TODO: YOUR CODE HERE
        if (mode == Mode.DRAW) {
            comm.sendRequest("draw" + " "+ curr.toString());
            repaint();
        } else if (mode == Mode.DELETE) {
            repaint();
        }
